...
function encodeToRot13(str) {
  const alphabetLIttle = 'abcdefghijklmnopqrstuvwxyz';
  ...
  str.split('').forEach((char) => {
    ...
    let currentAlphabet = alphabetLIttle;

    if (char.toUpperCase() === char) {
      currentAlphabet = alphabetBig;
    }

    const initialIndex = currentAlphabet.indexOf(char);
    let targetIndex = initialIndex + 13;
    if (targetIndex > currentAlphabet.length - 1) {
      const requiredPosition = initialIndex + 13;
      const ROT13position = requiredPosition - currentAlphabet.length;

      targetIndex = ROT13position;
    }

    final += currentAlphabet[targetIndex];
  });
  return final;
}
